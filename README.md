...
#### Testing
 * Download our pretrained model from [OneDrive](https://1drv.ms/f/s!AqddfvhavTRih3n3W0P29cxVIlfM) and move ```ironboiiz_060_00463920.pt``` to ```checkpoints/ironboiiz/```. 
 * Evaluate the model performance by ```python test_ironboiiz.py --name ironboiiz -r --icnn_path checkpoints/ironboiiz/ironboiiz_060_00463920.pt --hyper```

#### Training
* Reproduce our results by ```python train_ironboiiz.py --name ironboiiz --hyper``` 
* Check ```options/ironboiiz/train_options.py``` to see more training options. 

### 3. Playing with unaligned data
* Reproduce our finetuned model by ```python train_ironboiiz_unaligned.py --name ironboiiz_unaligned_ft --hyper -r --icnn_path checkpoints/ironboiiz/ironboiiz_060_00463920.pt --unaligned_loss vgg```
